...
**Purpose:** Generate a pool of random mazes of varying sizes and persist them to `maze_pool/` for later use by the live demo.

**Functions:**

- `generate_pool(sizes: list[int] = [5, 10, 15], n_per_size: int = 3, base_seed: int = 100, out_dir: str = "maze_pool") -> list[str]`
...
- `if __name__ == "__main__":` — argparse with `--sizes` (nargs, default `[5, 10, 15]`), `--n-per-size` (default 3), `--base-seed` (default 100), `--out-dir` (default `"maze_pool"`). Calls `generate_pool(...)` and prints how many files were written.

**Run command:** `python -m maze.pool_generator` (or `python maze/pool_generator.py` thanks to the `sys.path` pattern used elsewhere — follow `tests/` convention if running as a script directly).
...
**Purpose:** Pygame demo — pick a maze from `maze_pool/`, show it statically, wait for SPACE, then train Q-Learning / SARSA / Dyna-Q live (from scratch) on that exact maze with rendering, and print a convergence comparison.

**Functions:**

- `pick_maze_file(maze_pool_dir: str = "maze_pool", path: str = None) -> str`
  - If `path` given, return it directly.
  - Else load `{maze_pool_dir}/index.json` and pick a random entry (`np.random.choice` or `random.choice`). Raise a clear error if `maze_pool/` is empty (instruct user to run `python -m maze.pool_generator` first).
...
### `tests/phase6_unseen_maze.py` (new)
Following the style of `tests/phase3_dqn.py` (UTF-8 stdout reconfigure, `sys.path` insert, `PASS:` prints).

**Tests:**

1. `test_pool_generation()` — call `generate_pool(sizes=[5], n_per_size=1, out_dir="maze_pool/_test_pool")`, assert file exists, JSON has `size`, `seed`, `start`, `goal`, `grid` keys, `grid` shape matches `(2*5+1, 2*5+1)`. Clean up `_test_pool` dir after.
...
8. `tests/phase6_unseen_maze.py` — write and run; fix any issues in steps 1-7 before moving on.
9. `demo_unseen_maze.py` — live demo script (depends on everything above).
10. Run `python -m maze.pool_generator` to populate `maze_pool/`, then `python demo_unseen_maze.py` for a manual smoke test (visual — cannot be automated).

---

## Acceptance Check

- [ ] `python -m maze.pool_generator` creates `maze_pool/index.json` and `(len(sizes) * n_per_size)` JSON files.
- [ ] `python tests/phase6_unseen_maze.py` → `Phase 6: ALL TESTS PASS`.
